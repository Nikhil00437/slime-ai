...
import { TensuraSkill, RANK_META, SkillRank } from './types';
...
function parseClaudeDesktopSkill(raw: string, filename: string): TensuraSkill {
  ...
  return {
    ...
    icon: '📜',
    rank: 'normal', // Claude Desktop skills start as Normal — model ranks on import
...
function parseForgeSkillJSON(raw: string, filename: string): TensuraSkill {
  let parsed: any;
  try {
...
const RANK_FILTERS: Array<SkillRank | 'all'> = ['all', 'normal', 'rare', 'unique', 'ultimate'];

export const SkillForge: React.FC = () => {
  const { providers, activeModel } = useAppContext();

  const [skills, setSkills] = useState<TensuraSkill[]>([
    {
      id: 'gluttony',
      name: 'Gluttony',
  ...
  const [detailSkill, setDetailSkill] = useState<TensuraSkill | null>(null);
  ...
  const handleExport = useCallback((skill: TensuraSkill) => {
  ...
  const handleMerge = useCallback(async () => {
    ...
    const mergedSkill: TensuraSkill = {
